...
<div class="max-w-3xl mx-auto px-4 py-10">
    ...
    {#if formSubmitted}
        <div class="rounded-lg border border-green-200 bg-green-50 p-6 text-green-800 shadow text-center">
    ...
    {:else}
        <form class="space-y-6 rounded-xl bg-white p-6 shadow-md" on:submit|preventDefault={submitForm}>
            ...
            {#if step === 1}
                <div>
                    <h2 class="text-xl font-semibold text-[#008751] mb-4">Step 1: Personal Information</h2>
            ...
            {#if step === 2}
                <div>
                    <h2 class="text-xl font-semibold text-[#008751] mb-4">Step 2: Contact Information</h2>
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        <input type="email" bind:value={email} placeholder="Email Address *" required class="input" />
                        <input type="tel" bind:value={phone} placeholder="Phone Number *" required class="input" />
                    </div>
                </div>
            {/if}
            ...
            {#if step === 3}
                <div>
                    <h2 class="text-xl font-semibold text-[#008751] mb-4">Step 3: Identification</h2>
                    <input type="text" bind:value={nin} placeholder="National Identification Number (NIN) *" required maxlength="11" class="input" />
                </div>
            {/if}
            ...
            {#if step === 4}
                <div>
                    <h2 class="text-xl font-semibold text-[#008751] mb-4">Step 4: Location & Verification</h2>
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    ...
                    <textarea bind:value={address} placeholder="Full Address *" required class="input min-h-[100px] w-full mt-4"></textarea>

                    <!-- Captcha -->
                    <div class="flex flex-col sm:flex-row sm:items-center sm:space-x-4 mt-4">
                        <label for="captcha-input" class="text-sm font-medium text-gray-700 mb-1 sm:mb-0">
                            Verify you're not a bot:
                        </label>
                        <span class="text-lg font-semibold text-[#008751]">{num1} + {num2} =</span>
                        <input id="captcha-input" type="text" bind:value={captchaAnswer} placeholder="?" class="input w-24" required />
                    </div>

                    {#if captchaAnswer && !captchaCorrect}
                        <p class="text-sm text-red-600 mt-1">Incorrect captcha answer. Please try again.</p>
                    {/if}
                </div>
            {/if}
            ...
            <div class="flex justify-between pt-6">
                {#if step > 1}
                    <button type="button" on:click={prevStep} class="text-[#008751] hover:underline">← Back</button>
                {/if}
                {#if step < 4}
                    <button type="button" on:click={nextStep} class="rounded bg-[#008751] px-6 py-2 text-white hover:bg-[#006f42]">Next →</button>
                {:else}
                    <button type="submit" class="rounded bg-[#008751] px-6 py-2 text-white hover:bg-[#006f42]">Submit</button>
                {/if}
            </div>
        </form>
    {/if}
